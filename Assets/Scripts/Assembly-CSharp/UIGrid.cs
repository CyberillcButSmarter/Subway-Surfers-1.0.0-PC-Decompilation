		...
		else
		{
			for (int k = 0; k < transform.childCount; k++)
			{
				...
				if (child.gameObject.active || !hideInactive)
				{
					...
					child.localPosition = ((arrangement != Arrangement.Horizontal) ? new Vector3(cellWidth * (float)num2, (0f - cellHeight) * (float)num, z2) : new Vector3(cellWidth * (float)num, (0f - cellHeight) * (float)num2, z2));
					if (++num >= maxPerLine && maxPerLine > 0)
					{
						num = 0;
						num2++;
